...
CLASS_ICON_TCOORDS = {
	...
	["WITCHDOCTOR"] = {0.75 , 0.875, 0    , 0.125},
	["DEMONHUNTER"]	= {0.25 , 0.375, 0.125, 0.25 },
	["WITCHHUNTER"]	= {0.875, 1    , 0    , 0.125},
	["STORMBRINGER"]= {0    , 0.125, 0.125, 0.25 },
	["FLESHWARDEN"]	= {0.75 , 0.875  , 0.125, 0.25 },
	["ABOMINATION"]	= {0.75 , 0.875, 0.125, 0.25 },
	["GUARDIAN"]	= {0.25 , 0.375, 0    , 0.125},
	
	["MONK"]	    = {0.5  , 0.625, 0.125, 0.25 },
	["SONOFARUGAL"]	    = {0.25 , 0.375, 0.25 , 0.375},
	["THIEF"]	    = {0.5  , 0.625, 0.25 , 0.375},
	["RANGER"]	    = {0.125, 0.25 , 0    , 0.125},
	["BARD"]	    = {0.375, 0.5  , 0    , 0.125},
	["PROPHET"]	    = {0.625, 0.75 , 0, 0.125},
	["CHRONOMANCER"]	    = {0.625, 0.75 , 0.25 , 0.375},
	["PYROMANCER"]	= {0.5  , 0.625, 0    , 0.125},
	["CULTIST"]	= {0.375, 0.5  , 0.125, 0.25 },
	["NECROMANCER"]	= {0.625, 0.75, 0.375, 0.5},

	
	["SUNCLERIC"]	= {0.125, 0.25 , 0.375, 0.5  },
	["TINKER"]	    = {0.125, 0.25 , 0.25 , 0.375},
	["RIFTBLADE"]	= {0.625, 0.75 , 0.125, 0.25 },
	["REAPER"]	    = {0.375, 0.5  , 0.25 , 0.375},
	["WILDWALKER"]	= {0.5  , 0.625, 0.375, 0.5  },
	["STARCALLER"]	    = {0  , 0.125, 0.5, 0.625},
	["TIDECALLER"]	= {0.125, 0.25 , 0.5, 0.625},
	...
	["SPIRITMAGE"]	= {0.125, 0.25 , 0.125, 0.25},
	["FREE"]	= {0.49609375, 0.7421875, 0.5, 0.75},
};
...
function CharacterCreate_OnShow()
	...
	CharacterChangeFixup();
	 ...
	 if (DEV_REALM) then
		SetSelectedClass(10) -- selecting a race would have changed class to default
		SetCharacterClass(10) -- druid
	end
...
function CharacterCreateEnumerateClasses(...)
	...
	local classesToHide = 10 -- hide regular classes and display only custom ones

	if (CLASSLESS_REALM) then
		classesToHide = MAX_CLASSES_PER_RACE
	end

	for i = 1, classesToHide do
		_G["CharCreateClassButton"..i]:Hide()
	end

	--[[local saved_y = nil
	local saved_x = nil
	for i = classesToHide, MAX_CLASSES_PER_RACE do
		local mid = (MAX_CLASSES_PER_RACE-classesToHide)/2


		if (i == (MAX_CLASSES_PER_RACE-2)) then
			saved_x, saved_y = CalcPos(START_ANGLE_BACK + (ANGLE*(mid-(i-classesToHide))), RADIUS)
		end

		if (i < (classesToHide + mid)) then
			_G["CharCreateClassButton"..i]:SetPoint("CENTER", CalcPos((START_ANGLE + ANGLE*(i-classesToHide)), RADIUS))
		elseif (i < (MAX_CLASSES_PER_RACE-1)) then
			_G["CharCreateClassButton"..i]:SetPoint("CENTER", CalcPos(START_ANGLE_BACK + (ANGLE*(mid-(i-classesToHide))), RADIUS))
		elseif (i == (MAX_CLASSES_PER_RACE-1)) then -- spirit mage exception
			local x = CalcPos(270, RADIUS)
			_G["CharCreateClassButton"..i]:SetPoint("CENTER", x, saved_y)
		end
	end]]--
end
