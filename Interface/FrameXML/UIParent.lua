...
local function MSGR_Create(prefix)
...
local function MSGR_Request(prefix, msg)
	SendAddonMessage(prefix, msg, "WHISPER", UnitName("player"))
end

ScreenBlockFrameAIO = CreateFrame("FRAME", "ScreenBlockFrameAIO", nil, nil)
local AIOCheckHandler = MSGR_Create("SMSG_AIO_CHECK")

function AIOCheckHandler:Handle(msg)
	if not (AIO) then
		UIParent:Hide()
		ScreenBlockFrameAIO:Show()
		return
	end
end

local StatusBarAddonHandler = MSGR_Create("SMSG_STATUSBAR_CHECK")

function StatusBarAddonHandler:Handle(msg)
	if (G_ENERGYRAGE) then
		RunEnergyRage()
	end
end

function AIO_ActionTaker()
	EnableAddOn("AIO_Client")
	ReloadUI()
end

ScreenBlockFrameAIO:SetBackdrop({bgFile = "Interface/Tooltips/UI-Tooltip-Background", 
                                            edgeFile = "Interface/Tooltips/UI-Tooltip-Border", 
                                            tile = true, tileSize = 16, edgeSize = 16, 
                                            insets = { left = 4, right = 4, top = 4, bottom = 4 }});
ScreenBlockFrameAIO:SetBackdropColor(0,0,0,1);
ScreenBlockFrameAIO:SetPoint("CENTER")
ScreenBlockFrameAIO:SetSize(1920, 1080)
ScreenBlockFrameAIO:EnableMouse(true)
ScreenBlockFrameAIO:EnableKeyboard(true)
ScreenBlockFrameAIO:SetFrameStrata("TOOLTIP")
ScreenBlockFrameAIO:SetScript("OnKeyUp", function()
	--ForceQuit()
	AIO_ActionTaker();
end)
ScreenBlockFrameAIO:SetScript("OnMouseUp", function()
	--ForceQuit()
	AIO_ActionTaker();
end)

ScreenBlockFrameAIO.AlertBox = CreateFrame("Frame", "ScreenBlockFrameAIOAlertBox",ScreenBlockFrameAIO,nil)
ScreenBlockFrameAIO.AlertBox:ClearAllPoints()
ScreenBlockFrameAIO.AlertBox:SetBackdrop({bgFile = "Interface/Tooltips/UI-Tooltip-Background", 
                                            edgeFile = "Interface/Tooltips/UI-Tooltip-Border", 
                                            tile = true, tileSize = 16, edgeSize = 16, 
                                            insets = { left = 4, right = 4, top = 4, bottom = 4 }});
ScreenBlockFrameAIO.AlertBox:SetBackdropColor(0,0,0,1);
ScreenBlockFrameAIO.AlertBox:SetHeight(105)
ScreenBlockFrameAIO.AlertBox:SetWidth(390)
ScreenBlockFrameAIO.AlertBox:SetPoint("CENTER")

ScreenBlockFrameAIO.AlertBox.text = ScreenBlockFrameAIO.AlertBox:CreateFontString(nil, "BACKGROUND", "GameFontHighlight")
ScreenBlockFrameAIO.AlertBox.text:SetFont("Fonts\\FRIZQT__.TTF", 13)
ScreenBlockFrameAIO.AlertBox.text:SetFontObject(GameFontNormal)
--ScreenBlockFrameAIO.AlertBox.text:SetText("|cffFF0000WARNING!|r\nYour AIO AddOn is disabled.\nTo enable AIO please do following:\n|cffFFFFFF1. Go to AddOns at character select screen\n2. Enable Load out of date AddOns\n3. Enable AIO Addon|r\nPress any key to exit.")
ScreenBlockFrameAIO.AlertBox.text:SetText("|cffFF0000WARNING!|r\n\n   The required addons are disabled.\n|cffFFFFFF   Press any key to re-enable them.")
ScreenBlockFrameAIO.AlertBox.text:SetPoint("TOP",0,-20)

ScreenBlockFrameAIO.AlertBox.Alert = ScreenBlockFrameAIO.AlertBox:CreateTexture("ScreenBlockFrameAIOAlertBoxAlert") 
ScreenBlockFrameAIO.AlertBox.Alert:SetTexture("Interface\\DialogFrame\\UI-Dialog-Icon-AlertNew") 
ScreenBlockFrameAIO.AlertBox.Alert:SetSize(48,48)
ScreenBlockFrameAIO.AlertBox.Alert:SetPoint("LEFT",22,-5)

ScreenBlockFrameAIO:Hide()