...
-- @robinsch: Ascension Custom Maps
ASCENSION_WORLDMAP_TABLE = {}
ASCENSION_WORLDMAP_TABLE["Stormwind"] = 2
ASCENSION_WORLDMAP_TABLE["Elwynn"] = 2
ASCENSION_WORLDMAP_TABLE["DunMorogh"] = 2
ASCENSION_WORLDMAP_TABLE["Teldrassil"] = 1
ASCENSION_WORLDMAP_TABLE["Orgrimmar"] = 1
ASCENSION_WORLDMAP_TABLE["Durotar"] = 1
ASCENSION_WORLDMAP_TABLE["Mulgore"] = 1
ASCENSION_WORLDMAP_TABLE["Tirisfal"] = 2
ASCENSION_WORLDMAP_TABLE["StranglethornVale"] = 2
...
function WorldMapFrame_Update()
	...
	-- @robinsch: Ascension Custom Maps
	zoomOutAvailable = IsZoomOutAvailable() or ASCENSION_WORLDMAP_TABLE[mapFileName]

	-- Enable/Disable zoom out button
	if ( zoomOutAvailable ) then
...
function WorldMapZoomOutButton_OnClick()
	...
	currentMapContinent = GetCurrentMapContinent()
	if ASCENSION_WORLDMAP_TABLE[mapFileName] then
		currentMapContinent = ASCENSION_WORLDMAP_TABLE[mapFileName]
	end

	if ( GetCurrentMapZone() ~= WORLDMAP_WORLD_ID or ASCENSION_WORLDMAP_TABLE[mapFileName] ) then
		SetMapZoom(currentMapContinent);
