...
function GameTooltip_OnLoad(self)
	...
	if (self:HasScript('OnTooltipSetItem')) then
		self:HookScript('OnTooltipSetItem', GameTooltip_ItemVersionReplacer);
	end
end

function GameTooltip_ItemVersionReplacer(tooltip)
	local indexOfHeroicText = -1;
	local indexOfFlavorText = -1;
	...
	-- @HelloKitty: We must locate the lines that involve "Heroic" version tag and the flavor text line.
	-- @ is the special metadata character in flavor text to indicate a custom green text
	for index = 1, tooltip:NumLines() do
		if (_G[name .. index] ~= nil and _G[name .. index]:GetText() ~= nil) then
			if (_G[name .. index]:GetText() == ITEM_HEROIC) then
				indexOfHeroicText = index;
			else
				if (_G[name .. index]:GetText():match('@')) then
					indexOfFlavorText = index;
				end
			end
		end	
	end
	
	if (indexOfHeroicText < 0 or indexOfFlavorText < 0) then
		return;
	end	

	-- We need to find a Regex match of all characters bewtween the @ @. Flavor texts use "" also around the string
	-- Once found we can simply set flavor text to empty-string and the WoW UI handles sizing correctly as if it was never there
	-- TODO: Must support removing the Regex Match from the flavor text if there is more in flavor text than just the match so we can preserve flavor texts
	-- if they contain more than just metadata.
	local flavor = _G[name .. indexOfFlavorText]:GetText():match('@(.+)@');
	if (flavor ~= nil) then
		local newDescription = string.gsub(_G[name .. indexOfFlavorText]:GetText(), "@"..flavor.."@", "");
		if ( newDescription ~= "\"\"") then
			_G[name .. indexOfFlavorText]:SetText(newDescription);
		else
			_G[name .. indexOfFlavorText]:SetText("");
		end
		return _G[name .. indexOfHeroicText]:SetText(flavor);
	end
end
