...
SERVER_ALERT_PTR_URL = "https://api.ascension.gg/api/article/twitter"
...
GlueDialogTypes["REMEMBER_PASSWORD"] = {
	text = "Voulez-vous vraiment enregistrer votre mot de passe? \nCe n'est pas sûr si quelqu'un a accès à vos fichiers car ils sont enregistrés en texte brut!",
	button1 = OKAY,
	button2 = CANCEL,
	OnAccept = function ()
	end,
...
function AccountLogin_OnShow(self)
	...
	AccountLoginPasswordEdit:SetText(dec(password) or "");
	if accountName ~= "" then
...
function enc(data)
  return ((data:gsub('.', function(x) 
    local r,b='',x:byte()
    for i=7,1,-1 do r=r..(b%2^i-b%2^(i-1)>0 and '1' or '0') end
    return r;
  end)..'0000'):gsub('%d%d%d?%d?%d?', function(x)
    if (#x < 5) then return '' end
    local c=0
    for i=1,5 do c=c+(x:sub(i,i)=='1' and 2^(5-i) or 0) end
...
function AccountLogin_Login()
	...
	if ( AccountLoginSaveAccountName:GetChecked() ) then
		if ( AccountLoginSavePassword:GetChecked() ) then
			SaveAccountString(AccountLoginAccountEdit:GetText(), enc(AccountLoginPasswordEdit:GetText()));
		else
